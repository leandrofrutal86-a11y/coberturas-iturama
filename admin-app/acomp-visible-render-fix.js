...
function ready(){return !!(document.getElementById('thAcomp1')&&document.getElementById('tbAcomp1'))}
...
function message(txt){if(!ready())return;const tb=document.getElementById('tbAcomp1');if(tb)tb.innerHTML=`<tr><td colspan="13" style="padding:14px 6px;text-align:center;font-weight:900;font-size:11px">${esc(txt)}</td></tr>`}
function renderNow(d=localData()){if(!d?.individual?.length||!d?.categorias?.length||!ready())return false;cached=d;window.__adminDash=d;const th=document.getElementById('thAcomp1'),tb=document.getElementById('tbAcomp1'),inds=(d.individual||[]).slice(),list=entries(d);th.innerHTML=head(inds);tb.innerHTML=list.map(e=>row(e,inds)).join('')||'<tr><td colspan="13" style="padding:14px;text-align:center">SEM CATEGORIAS PARA EXIBIR</td></tr>';try{window.invalidateAcompShare?.()}catch{}return true}
