...
function renderEntries(){
 ...
 $('ctCEntries').innerHTML=rows.length?rows.map(x=>'<article class="ctCCard"><div class="ctCCardTop"><div><b>PV '+E(x.pv)+'</b><h4>'+E(x.descricao)+'</h4><small>'+E(new Date(x.data+'T12:00:00').toLocaleDateString('pt-BR'))+'</small></div><button class="ctCEdit" data-ct-edit="'+x.id+'">EDITAR</button></div><div class="ctCNumbers"><div class="ctCNum">VOLUME<b>'+Number(x.volume||0).toLocaleString('pt-BR')+'</b></div><div class="ctCNum">VALOR/CX<b>'+money(x.valor_caixa)+'</b></div><div class="ctCNum">TOTAL<b>'+money(x.total)+'</b></div></div></article>').join(''):'<div class="ctCEmpty">Você ainda não criou contratos neste mês.</div>';
