...
.pvDiaDayStat{font-size:11px;font-weight:900;color:#33495d;background:#eef4f8;border-radius:9px;padding:7px 9px;margin:8px 0}.pvDiaHint{font-size:11px;color:#5f6d79;margin:8px 0}.pvDiaReportBtn{width:100%;border:0;border-radius:10px;padding:12px;background:#172534;color:#fff;font-weight:950}.pvDiaReportBtn:disabled{opacity:.6}.pvDiaLoading{padding:10px;text-align:center;color:#647482;font-weight:800}
...
async function setSelectedRoute(v){
 ...
 clients=[];baseClients=[];lastKey='';baseKey='';renderPanel();await Promise.all([getBaseClients(true),getDayClients(true)]);
}
function renderPanel(){
 ...
 const cliOpts=baseClients.map(c=>`<option value="${esc(c.pv)}">${effectiveRoute()==='TODOS'?esc(c.rota)+' • ':''}${esc(c.pv)} • ${esc(c.razao)}</option>`).join('');
 ...
 ${routePicker}
 <div class="pvDiaFilters"><div><label>${effectiveRoute()==='TODOS'?'CLIENTES DE TODAS AS ROTAS / BASE GERAL':'CLIENTES DA ROTA / BASE GERAL'}</label><select id="pvDiaClient" class="pvDiaSelect" ${baseLoading?'disabled':''}><option value="">${baseLoading?'Carregando Base Geral...':'Selecione um cliente...'}</option>${cliOpts}</select></div></div>
 <div class="pvDiaHint">A pesquisa usa todos os clientes da Base Geral da rota. O dia da visita abaixo altera somente os clientes programados e o relatório.</div>
...
function fallbackClient(pv){return baseClients.find(x=>String(x.pv)===String(pv))||clients.find(x=>String(x.pv)===String(pv))}
...
async function inject(force=false){
 ...
 renderPanel();
 if(isRogerioMulti())window.__pvSearchRoute=effectiveRoute();
 const rt=effectiveRoute(),dayKey=rt+'|'+selectedDay;
 const jobs=[];
 if(force||rt!==baseKey||!baseClients.length)jobs.push(getBaseClients(force));
 if(force||dayKey!==lastKey||!clients.length)jobs.push(getDayClients(force));
...
window.addEventListener('iturama:routechange',()=>{clients=[];baseClients=[];lastKey='';baseKey='';inject(true).catch(()=>{})});
