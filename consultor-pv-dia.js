...
function renderReport(data){
 ...
 <div class="pvDayClientGrid">${rows.length?rows.map(c=>`<article class="pvDayClientCard">
   ...
   ${c._faltas.length?`<div class="pvDayMissingTitle">O QUE FALTA</div><div class="pvDayMissingList">${c._faltas.map(x=>`<div class="pvDayMissingRow"><b>${esc(x.name)}</b><span>${esc(x.detail)}</span></div>`).join('')}</div>`:''}
