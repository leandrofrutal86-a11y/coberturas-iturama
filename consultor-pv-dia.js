...
function missingInfo(x){
 ...
 if(miss.length){
  ...
  return{name:String(x.nome||''),detail:names.join(' • '),simple:false};
 }
 const fq=Number(x?.falta_quantidade||0),fc=Number(x?.falta_compras||0);
 if(fq>0)return{name:String(x.nome||''),detail:`Falta ${fq} cx`,simple:false};
 if(fc>0)return{name:String(x.nome||''),detail:`Falta ${fc} compra${fc>1?'s':''}`,simple:false};
 return{name:String(x?.nome||''),detail:'✕',simple:true};
}
...
function renderReport(data){
 ...
 <div class="pvDayClientGrid">${rows.length?rows.map(c=>`<article class="pvDayClientCard">
   ...
   ${c._faltas.length?`<div class="pvDayMissingTitle">O QUE FALTA</div><div class="pvDayMissingList">${c._faltas.map(x=>`<div class="pvDayMissingRow ${x.simple?'simple':''}"><b>${esc(x.name)}</b><span>${esc(x.detail)}</span></div>`).join('')}</div>`:''}
...
function printReport(){
 ...
 @page{size:A4 landscape;margin:6mm}*{box-sizing:border-box}body{font-family:Arial,sans-serif;color:#111;margin:0}h1{font-size:15px;margin:0 0 2px}p{font-size:9px;margin:0 0 6px}.grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:5px}.pcard{border:1px solid #aaa;border-radius:6px;padding:5px;break-inside:avoid}.phead{display:flex;justify-content:space-between;gap:5px;align-items:center;font-size:9px}.ord{display:inline-block;background:#222;color:#fff;border-radius:999px;padding:2px 5px}.bad,.good{font-size:7px;font-weight:bold;border-radius:999px;padding:2px 5px}.bad{background:#fde0e0;color:#a00}.good{background:#def3e7;color:#075}.pname{font-size:10px;font-weight:bold;margin-top:3px}.psub{font-size:7px;color:#555;margin:1px 0 4px}.pmiss{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:3px}.pitem{border:1px solid #e5caca;background:#fff5f5;border-radius:4px;padding:3px;min-height:24px}.pitem b{display:block;font-size:6.8px;color:#900;line-height:1.05}.pitem span{display:block;font-size:6.8px;line-height:1.05;margin-top:1px}.pitem.simple{display:flex;align-items:center;justify-content:space-between;gap:3px}.pitem.simple b{margin:0}.pitem.simple span{font-size:10px;font-weight:bold;color:#d00}
 </style></head><body><h1>Relatório de visitas • ${esc(reportData.route)} • ${esc(dayLabel(reportData.day))}</h1><p>${rows.length} clientes • ${opp} com oportunidade • ${rows.length-opp} cobertos</p><div class="grid">${cards}</div><script>window.onload=()=>setTimeout(()=>window.print(),200)<\/script></body></html>`);w.document.close();
}
