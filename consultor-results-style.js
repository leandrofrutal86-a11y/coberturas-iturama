...
#results .res-ok{color:#0b8a46;font-weight:900}#results .res-bad{color:#d71920;font-weight:900}#results .meta-num{font-weight:900}#results .falta-num{font-weight:900;color:#c5161d}#results .pct-ok{color:#0b8a46;font-weight:900}#results .pct-bad{color:#d71920;font-weight:900}#results .dias-uteis{font-weight:900;color:#475569}#results .meta-dia{font-weight:950;color:#075fae;font-size:12px}
#results .daily-note{margin-top:7px;padding:7px 9px;border-radius:9px;background:#eef5fb;color:#28465f;font-size:9px;font-weight:800;text-align:center}
...
@media(max-width:760px){
 ...
 #results .tableWrap{border:0!important;box-shadow:none!important;overflow:visible!important;margin-top:5px!important}#results table{display:block!important;width:100%!important;max-width:100%!important;min-width:0!important}#results thead{display:none!important}#results tbody{display:grid!important;gap:5px!important;width:100%!important}
 #results tbody tr{display:grid!important;grid-template-columns:repeat(6,minmax(0,1fr))!important;width:100%!important;border:1px solid #d8e0e6!important;border-radius:9px!important;overflow:hidden!important;background:#fff!important}
 #results tbody td{display:block!important;min-width:0!important;padding:4px 1px!important;border:0!important;border-right:1px solid #e1e7ec!important;background:#fff!important;text-align:center!important;font-size:9px!important;line-height:1.05!important}
 #results tbody td:first-child{grid-column:1/-1!important;padding:5px 6px!important;background:#eef5fb!important;border-right:0!important;border-bottom:1px solid #d8e0e6!important;text-align:left!important;font-size:9px!important;white-space:normal!important;overflow-wrap:anywhere!important}
 #results tbody td:last-child{border-right:0!important}#results tbody td:not(:first-child)::before{content:attr(data-label);display:block;font-size:5.7px;line-height:1;color:#657380;font-weight:900;margin-bottom:2px;text-transform:uppercase;white-space:normal}
 #results .meta-dia{font-size:10px!important}.daily-note{font-size:8px!important;margin-top:5px!important}
 ...
 #crResultado{padding:0 6px 7px}.crTableWrap{border:0;overflow:visible}.crTable{display:block;width:100%;min-width:0}.crTable thead{display:none}.crTable tbody{display:grid;gap:4px}.crTable tr{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));border:1px solid #d8e0e6;border-radius:8px;overflow:hidden}.crTable td{display:block;padding:4px 1px;border:0;border-right:1px solid #e2e7eb;background:#fff!important;text-align:center;font-size:8px}.crTable td:first-child{grid-column:1/-1;padding:5px;background:#f0f6fa!important;border-right:0;border-bottom:1px solid #d8e0e6;text-align:left;font-size:8.5px}.crTable td:not(:first-child)::before{content:attr(data-label);display:block;font-size:5.5px;color:#657380;font-weight:900;margin-bottom:2px;text-transform:uppercase}.crTable td:last-child{border-right:0}
}
`;
...
function decorateResults(){
  ...
  if(meu&&head&&head.cells.length===5){let th=document.createElement('th');th.textContent='Dias úteis';head.appendChild(th);th=document.createElement('th');th.textContent='Meta/dia';head.appendChild(th)}
  ...
  table.querySelectorAll('tbody tr').forEach(tr=>{
    ...
    if(meu&&tr.cells.length===5){const a=tr.insertCell();a.className='dias-uteis';a.textContent=String(dias);const b=tr.insertCell();b.className='meta-dia';b.textContent=String(metaDiaria(falta,dias))}
    const labs=['','Meta','Real','Falta','%','Dias','Meta/dia'];[...tr.cells].forEach((c,i)=>{if(i>0)c.dataset.label=labs[i]||''})
  });
  let note=results.querySelector('.daily-note');if(meu){if(!note){note=document.createElement('div');note.className='daily-note';table.parentElement?.insertAdjacentElement('afterend',note)}note.textContent=`Meta diária: ${dias} dias úteis restantes. Segunda a sexta, sem contar o último dia do mês.`}else note?.remove();
...
function ensureReport(){
  ...
  const box=document.createElement('section');box.id='consultorRelatorio';box.innerHTML=`<div class="crHead"><div><b>RELATÓRIO INDIVIDUAL</b><br><small>META • REALIZADO • FALTA • DIAS ÚTEIS • META/DIA</small></div><small>SEGUNDA A SEXTA • ÚLTIMO DIA FORA</small></div><div class="crBtns"><button id="crGerar" class="crGen">GERAR RELATÓRIO</button><button id="crPdf" class="crPdf" disabled>📄 BAIXAR PDF</button><button id="crPrint" class="crPrint" disabled>🖨️ IMPRIMIR</button></div><div id="crStatus">Clique em GERAR RELATÓRIO.</div><div id="crResumo"></div><div id="crResultado"></div>`;
...
function gerarRelatorio(){
  ...
  if(!rows.length)$('crResultado').innerHTML='<div class="crEmpty">Nenhuma cobertura pendente. Meta concluída.</div>';else $('crResultado').innerHTML=`<div class="crTableWrap"><table class="crTable"><thead><tr><th>Categoria</th><th>Meta</th><th>Realizado</th><th>Falta</th><th>Dias</th><th>Meta/dia</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${esc(r.nome)}</td><td data-label="Meta">${r.meta}</td><td data-label="Real">${r.real}</td><td data-label="Falta">${r.falta}</td><td data-label="Dias">${r.dias}</td><td data-label="Meta/dia">${r.diaria}</td></tr>`).join('')}</tbody></table></div>`;
